...
if (isset($_POST['email']) || isset($_POST['file'])) {
    ...
    if (!empty($_FILES["file"]["name"])) {
        ...
        $allowTypes = array('jpg', 'png', 'jpeg', 'JPG', 'PNG', 'JPEG', 'gif', 'bmp');
        if (in_array($fileType, $allowTypes)) {
            ...
            $uploadStatus = 0;
            $response['message'] = 'Sorry, JPG, JPEG, PNG, GIF, BMP files are allowed to upload.';
        }
    }else{
        $uploadStatus = 0;
    }
    ...
    if ($uploadStatus == 1) {
        ...
        $insert = $db->query("UPDATE seekerinfo SET  picture='" . $uploadedFile . "' WHERE email='$email';");
